...
std::unordered_map<Opcode, std::set<std::unique_ptr<tvlm::Rule>>> tvlm::ILTiler::AllRulesInit(){
    std::unordered_map<Opcode, std::set<std::unique_ptr<Rule>>> all;
...
//    binRules.insert(std::make_unique<SpecializedRule>(Opcode::BinOp, 1, 2));
    ...
    children.push_back(new SpecializedRule(Opcode::LoadImm, 1));
